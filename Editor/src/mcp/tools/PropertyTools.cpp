...
// Curves have no PropertyBag primitive, so describe_component reports one as a
// compact read-only summary: "3 keys: (0.00,1.00) (0.50,0.80) (1.00,0.00)".
// Enough for an agent to see the shape without inventing a writable encoding.
std::string DescribeCurve(const AnimationCurve& c) {
    const auto& keys = c.Keys();
    if (keys.empty()) return "empty";
    std::ostringstream os;
    os << keys.size() << (keys.size() == 1 ? " key: " : " keys: ") << std::fixed << std::setprecision(2);
...
PropertyBag BuildComponentBag(Component* component) {
    ...
    if (auto* t = dynamic_cast<Transform*>(component)) {
        ...
        b.Float("emission_rate",[p]{return p->GetEmissionRate();},[p](float v){p->SetEmissionRate(v);},0.0); b.Int("max_particles",[p]{return p->GetMaxParticles();},[p](int v){p->SetMaxParticles(v);},1); b.Float("duration",[p]{return p->GetDuration();},[p](float v){p->SetDuration(v);},0.0); b.Bool("looping",[p]{return p->GetLooping();},[p](bool v){p->SetLooping(v);}); b.Int("start_burst",[p]{return p->GetStartBurst();},[p](int v){p->SetStartBurst(v);},0); b.Vec2("lifetime",[p]{return p->GetLifetime();},[p](glm::vec2 v){p->SetLifetime(v);}); b.Vec2("speed",[p]{return p->GetSpeed();},[p](glm::vec2 v){p->SetSpeed(v);}); b.Float("direction",[p]{return p->GetDirection();},[p](float v){p->SetDirection(v);}); b.Float("spread",[p]{return p->GetSpread();},[p](float v){p->SetSpread(v);},0.0,360.0); b.Vec2("gravity",[p]{return p->GetGravity();},[p](glm::vec2 v){p->SetGravity(v);}); b.Float("damping",[p]{return p->GetDamping();},[p](float v){p->SetDamping(v);},0.0); b.Enum("space",[p]{return static_cast<int>(p->GetSpace());},[p](int v){p->SetSpace(static_cast<ParticleComponent::SimulationSpace>(v));},{"Local","World"}); b.Enum("simulation_backend",[p]{return static_cast<int>(p->GetSimulationBackend());},[p](int v){p->SetSimulationBackend(static_cast<ParticleComponent::SimulationBackend>(v));},{"OpenGLCompute","CUDA"}); b.Enum("shape",[p]{return static_cast<int>(p->GetShape());},[p](int v){p->SetShape(static_cast<ParticleComponent::Shape>(v));},{"Point","Circle","Box","Cone"}); b.Vec2("shape_size",[p]{return p->GetShapeSize();},[p](glm::vec2 v){p->SetShapeSize(v);}); b.Float("cone_angle",[p]{return p->GetConeAngle();},[p](float v){p->SetConeAngle(v);},0.0,180.0); b.String("sprite",[p]{return p->GetSpriteID();},[p](const std::string& v){p->SetSprite(v);},"Sprite"); b.Bool("flip_x",[p]{return p->GetFlipX();},[p](bool v){p->SetFlipX(v);}); b.Bool("flip_y",[p]{return p->GetFlipY();},[p](bool v){p->SetFlipY(v);}); b.Vec4("start_color",[p]{return p->GetStartColor();},[p](glm::vec4 v){p->SetStartColor(v);}); b.Vec4("end_color",[p]{return p->GetEndColor();},[p](glm::vec4 v){p->SetEndColor(v);}); b.Float("start_size",[p]{return p->GetStartSize();},[p](float v){p->SetStartSize(v);},0.0); b.Float("end_size",[p]{return p->GetEndSize();},[p](float v){p->SetEndSize(v);},0.0); b.Enum("blend_mode",[p]{return static_cast<int>(p->GetBlendMode());},[p](int v){p->SetBlendMode(static_cast<ParticleComponent::BlendMode>(v));},{"Alpha","Additive"}); if(sortingLayers.empty())b.String("sorting_layer",[p]{return p->GetSortingLayer();},[p](const std::string& v){p->SetSortingLayer(v);});else b.StringChoice("sorting_layer",[p]{return p->GetSortingLayer();},[p](const std::string& v){p->SetSortingLayer(v);},sortingLayers); b.Int("sorting_order",[p]{return p->GetSortingOrder();},[p](int v){p->SetSortingOrder(v);});
    } else if (auto* t = dynamic_cast<TrailRenderer*>(component)) {
        // width_curve is exposed read-only: PropertyBag has no keyframe primitive,
        // and flattening a curve into a string a caller could write back would be
        // a parser nobody asked for. Authoring happens in the Inspector's curve
        // editor or from Python, where AnimationCurve is a real bound type.
        b.Float("time",[t]{return t->GetTime();},[t](float v){t->SetTime(v);},0.0); b.Float("min_vertex_distance",[t]{return t->GetMinVertexDistance();},[t](float v){t->SetMinVertexDistance(v);},0.0); b.Float("width_multiplier",[t]{return t->GetWidthMultiplier();},[t](float v){t->SetWidthMultiplier(v);},0.0); b.String("width_curve",[t]{return DescribeCurve(t->GetWidthCurve());}); b.Vec4("start_color",[t]{return t->GetStartColor();},[t](glm::vec4 v){t->SetStartColor(v);}); b.Vec4("end_color",[t]{return t->GetEndColor();},[t](glm::vec4 v){t->SetEndColor(v);}); b.String("material",[t]{return t->GetMaterialID();},[t](const std::string& v){t->SetMaterial(v);},"Material"); b.String("sprite",[t]{return t->GetSpriteID();},[t](const std::string& v){t->SetSprite(v);},"Sprite"); b.Bool("emitting",[t]{return t->GetEmitting();},[t](bool v){t->SetEmitting(v);}); b.Bool("autodestruct",[t]{return t->GetAutodestruct();},[t](bool v){t->SetAutodestruct(v);}); if(sortingLayers.empty())b.String("sorting_layer",[t]{return t->GetSortingLayer();},[t](const std::string& v){t->SetSortingLayer(v);});else b.StringChoice("sorting_layer",[t]{return t->GetSortingLayer();},[t](const std::string& v){t->SetSortingLayer(v);},sortingLayers); b.Int("sorting_order",[t]{return t->GetSortingOrder();},[t](int v){t->SetSortingOrder(v);}); b.Int("point_count",[t]{return static_cast<int>(t->GetPoints().size());});
        ...
        for (const ScriptFieldInfo& field : fields) {
            ...
            p.get=[s,fieldName]{ ScriptFieldValue v=s->GetFieldValue(fieldName); return std::visit([](const auto& x)->QJsonValue { using T=std::decay_t<decltype(x)>; if constexpr (kIsVector<T>) return VectorToJson(x); else if constexpr (std::is_same_v<T,AnimationCurve>) return QJsonValue(QString::fromStdString(DescribeCurve(x))); else return ToJson(x); },v); };
